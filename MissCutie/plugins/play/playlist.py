...
from MissCutie.utils.database import (delete_playlist, get_playlist,
                                       get_playlist_names,
                                       save_playlist)
from MissCutie.utils.decorators.language import language, languageCB
...
async def check_playlist(client, message: Message, _):
    ...
    msg = _["playlist_4"]
    count = 0
    for MissCutie in _playlist:
        _note = await get_playlist(message.from_user.id, MissCutie)
        title = _note["title"]
        title = title.title()
