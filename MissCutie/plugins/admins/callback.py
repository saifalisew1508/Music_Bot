...
from MissCutie import YouTube, app
from MissCutie.core.call import MissCutie
from MissCutie.misc import SUDOERS, db
from MissCutie.utils.database import (is_active_chat,
...
from MissCutie.utils.formatters import seconds_to_min
from MissCutie.utils.inline.play import (panel_markup_1,
                                          panel_markup_2,
                                          panel_markup_3,
                                          stream_markup,
                                          telegram_markup)
from MissCutie.utils.stream.autoclear import auto_clean
from MissCutie.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await MissCutie.mute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await MissCutie.unmute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await MissCutie.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await MissCutie.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await MissCutie.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
