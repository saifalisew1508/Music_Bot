...
from MissCutie.utils.database import autoend_off, autoend_on
...
async def auto_end_stream(client, message):
    ...
    if state == "enable":
        ...
        await message.reply_text(
            "Auto End Stream Enabled.\n\nBot will leave voice chat automatically after 5 mins if no one is listening with a warning message..."
        )
    elif state == "disable":
        await autoend_off()
